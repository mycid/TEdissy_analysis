...
# Step 1: mg/mL glucose equivalent
carb_weights$con_mg_per_ml <- (carb_weights$absorbance - intercept) / slope

# Step 2: Calculate mg glucose eq. in 30 µL aliquot
carb_weights$mg_in_aliquot <- carb_weights$con_mg_per_ml * 0.030

# Step 3: Scale to total extract volume (3.03 mL extract / 30 µL used = 33.33)
scaling_factor <- 3030 / 30  # 33.33
carb_weights$mg_in_extract <- carb_weights$mg_in_aliquot * scaling_factor

# Step 4: Calculate percent carbohydrate
carb_weights$percent_carb <- (carb_weights$mg_in_extract / carb_weights$weights) * 100
...
carb_weights_clean <- carb_weights %>% dplyr::filter(outlier_flag == "normal")
...
carb_rep_final <- carb_rep_final %>%
  dplyr::filter(!is.na(carb_mg_per_g_mean))
...
carb_location_cham <- carb_rep_final %>%
  dplyr::filter(!variety %in% c("F.Glom"))
