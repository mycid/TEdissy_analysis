...
calculate_pe_and_filter <- function(tidy_df, 
  ...
  # Step 0: Optionally simulate lower values at 455 and 592 nm
  ...
  # Step 1: Calculate PE in mg/mL using the corrected formula
  tidy_df <- tidy_df %>%
    mutate(
      PE_mg_per_mL = ((`X564` - `X592`) - ((`X455` - `X592`) * 0.20)) * 0.12
    )
  
  # Step 2: Identify rows with negative PE concentration (mg/mL)
  ...
  # Step 3 & 4: Calculate total PE in mg and PE per gram of sample
  ...
  # Attach removed rows as attribute for record keeping
...
#
#
joindf_by_id <- function(df1, df2, output_name, key_df1, key_df2, assign_to_global = TRUE) {
  ...
  # Assign to global environment (optional)
  ...
  # Print repeat list for manual review
  repeat_list <- unique(unmatched_df[[1]])
