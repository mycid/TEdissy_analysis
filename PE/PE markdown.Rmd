...
# Create "output PE/plots/" if it doesn’t exist
...
All eight Excel workbooks (four PE, four Fluorescence) are now loaded into R as data.frames named according to each file’s base name.
Any temporary “\~\$…” files were skipped.

## STEP 2: CONVERT RAW DATA INTO A TIDY FORMAT

-   We have eight “raw” data.frames (e.g., `PE1`, `PE2`, …, `Fluor4`).
-   We define which wells are blanks for each PE run (e.g., `A01`, `A01, A02, A03`, etc.) and each Fluorescence run.
-   We call a wrapper function `tidy_all()`—it:
    -   Calls `tidy_and_correct()` internally for each dataset.
    -   Reads raw absorbance/fluorescence values, reshapes them into “long” format.
...
-   After running these lines, you end up with eight new “\_tidy” data.frames in your environment.
...
-   We store these in a data.frame called `absorbance_ratio_df`, which has two columns:
    -   `ratio_type` (character/factor: “X455/X564” or “X592/X564”)
    -   `ratio_value` (numeric: the computed ratio for each well)
-   We then create a jitter plot of `ratio_value` versus `ratio_type` and draw a horizontal dashed line at y = 1.0 (the expected “no‐bias” line).
...
Aust J Mar Freshwater Res 36:785–792, <https://doi.org/10.1071/MF9850785>.
...
-   The steps are:
    1.  **Loop Over Each Fluorescence Run:**
        -   For each `Fluor#_tidy`, build a ggplot bar chart where:
            -   X‐axis is `Cell_ID` (the individual sample/well).\
            -   Y‐axis is `Xred` (fluorescence value).\
...
-   We want to merge each fluorescence dataset (`Fluor#_tidy`) with its corresponding PE‐calculation dataset (`PE#_calc`), so that each sample’s PE and Xred values are in the same table.
-   The steps are:
    ...
    4.  **Identify Common Columns and Bind Rows:**
        ...
        -   Keep only those common columns in each data frame before row‐binding.
    5.  **Fix Date Columns (American → European):**
        -   For any column that inherits `"Date"` or whose name contains “date”, attempt to parse as `"%m/%d/%Y"` first; if that fails, retry as `"%d/%m/%Y"`.
...
model_initial_run2 <- lm(PE_mg_per_g_sample ~ Xred, data = df_run2)
...
# (Optional) Visualize initial model with outliers highlighted
ggplot(df_run2, aes(x = Xred, y = PE_mg_per_g_sample, color = is_outlier)) +
...
df_run2_clean <- df_run2 %>%
  filter(!is_outlier)

model_clean_run2 <- lm(PE_mg_per_g_sample ~ Xred, data = df_run2_clean)
...
ggplot(df_run2_clean, aes(x = Xred, y = PE_mg_per_g_sample)) +
...
df_combined <- calc_PE_from_Xred(df_combined, fluor_col = "Xred", new_col = "PE_pred_run2_mg_per_g")
...
1.
**Filter to that run’s subset of data.**\
2.
...
$$
     ...
     $$\
4.
**Extract R² and p‐values** for each model using `get_stats()`.\
5.
**Build a ggplot** that:\
...
6.
**Save each run’s plot** under `plots/regression/<run>/Xred_PE_regressions_<run>.png`.\
7.
...
Without enhanced CV‐based selection (`choose_best_3 = FALSE`), saving as `all_rep_analy_summary.csv`.\
2.
...
-   Below are two sub‐sections demonstrating these scenarios.
...
# 📁 1. Create export folder for comparison results
comparison_dir <- file.path("PE", "output_PE", "export data", "PErep_comparisons")
dir.create(comparison_dir, recursive = TRUE, showWarnings = FALSE)

# 📊 2. Subset data: gametophyte/tetrasporophyte life stages
df_gamtetra <- PErep_final %>%
  filter(Life_S %in% c("Gam/Tetra", "Gam", "Tetra"))

# 📈 3. Compare PE across locations for gametophyte/tetrasporophyte
res_gamtetra_PE <- compare_groups(
  data         = df_gamtetra,
  response_var = "PE_mg_per_g_sample_mean",
  group_var    = "Location"
)
cat("### PE comparison by Location (Gam/Tetra)\n")
print(res_gamtetra_PE)
write.csv(res_gamtetra_PE, file = file.path(comparison_dir, "PE_by_location_gamtetra.csv"), row.names = FALSE)

# 📈 4. Compare Xred across locations for gametophyte/tetrasporophyte
res_gamtetra_Xred <- compare_groups(
  data         = df_gamtetra,
  response_var = "Xred_mean",
  group_var    = "Location"
)
cat("### Xred comparison by Location (Gam/Tetra)\n")
print(res_gamtetra_Xred)
write.csv(res_gamtetra_Xred, file = file.path(comparison_dir, "Xred_by_location_gamtetra.csv"), row.names = FALSE)

# 📊 5. Subset data: carpogonium life stage
df_carp <- PErep_final %>%
  filter(Life_S == "Carp")

# 📈 6. Compare PE across locations for carpogonium
res_carp_PE <- compare_groups(
  data         = df_carp,
  response_var = "PE_mg_per_g_sample_mean",
  group_var    = "Location"
)
cat("### PE comparison by Location (Carpogonium)\n")
print(res_carp_PE)
write.csv(res_carp_PE, file = file.path(comparison_dir, "PE_by_location_carp.csv"), row.names = FALSE)

# 📊 7. Subset data: C. chamissoi variety
df_cham <- PErep_final %>%
  filter(variety == "C.Cham")

# 📈 8. Compare PE across locations for C.Cham
res_cham_PE <- compare_groups(
  data         = df_cham,
  response_var = "PE_mg_per_g_sample_mean",
  group_var    = "Location"
)
cat("### PE comparison by Location (C.Cham variety)\n")
print(res_cham_PE)
write.csv(res_cham_PE, file = file.path(comparison_dir, "PE_by_location_cham.csv"), row.names = FALSE)

# 📊 9. Subset data: F. glomerata variety
df_glom <- PErep_final %>%
  filter(variety == "F. Glom")

# 📈 10. Compare PE across locations for F.Glom
res_glom_PE <- compare_groups(
  data         = df_glom,
  response_var = "PE_mg_per_g_sample_mean",
  group_var    = "Location"
)
cat("### PE comparison by Location (F. Glom variety)\n")
print(res_glom_PE)
write.csv(res_glom_PE, file = file.path(comparison_dir, "PE_by_location_glom.csv"), row.names = FALSE)

# 📊 11. Subset by selected locations only
df_selected_sites <- PErep_final %>%
  filter(Location %in% c("Mendieta", "7H", "Caro Caido"))

# 📈 12. Compare PE across selected sites
res_sites_PE <- compare_groups(
  data         = df_selected_sites,
  response_var = "PE_mg_per_g_sample_mean",
  group_var    = "Location"
)
cat("### PE comparison across Mendieta, 7H, and Caro Caido\n")
print(res_sites_PE)
write.csv(res_sites_PE, file = file.path(comparison_dir, "PE_by_location_selected_sites.csv"), row.names = FALSE)
```
##Step 14: Bootsrapped comparison
## Example: Bootstrapped Group Comparisons with `compare_groups_bootstrap`

In this chunk, we:

1. **Load** or attach your data (e.g. a CSV or whichever source you use).  
2. **Call** `compare_groups_bootstrap()` to compute 95% bootstrapped confidence intervals and effect sizes (mean differences and Cohen’s _d_) for a response variable across treatment groups.  
3. **Inspect** the returned summary tables (bootstrapped means + CIs, pairwise effect sizes + CIs).  
4. **Plot** the two ggplots (group‐means with CIs, and pairwise effect‐size estimates with CIs).  
5. Optionally, **arrange** both plots side‐by‐side using `patchwork`.

```{r}
# 1. Load your data into R. 
#    (Replace the path or method below with however you import your dataset.)
#    This should produce a data.frame named `PErep_final` with at least:
#      • A numeric column "PE_mg_per_g_sample_mean" 
#      • A factor/character column "Location"
PErep_final <- read.csv("path/to/your/PErep_final.csv")

# 2. Call compare_groups_bootstrap() to run bootstrapped analyses.
#    - data:       the data.frame (PErep_final)
#    - response_var: the numeric response column (PE_mg_per_g_sample_mean)
#    - group_var:    the factor/“treatment” column (Location)
#    - facet_var = NULL: means no faceting; run as a single overall analysis
#    - n_boot = 5000:       number of bootstrap resamples (adjust if speed is an issue)
#    - conf_level = 0.95:   95% confidence intervals
res <- compare_groups_bootstrap(
  data         = PErep_final,
  response_var = "PE_mg_per_g_sample_mean",
  group_var    = "Location",
  facet_var    = NULL,       
...
# 3. Inspect the structure of the returned summaries:
#    - `res$summary` is a named list. If no facet_var was used, it will have a single element "ALL".
names(res$summary)              
#    • Should print: "ALL"  

# 3a. View bootstrapped means and CIs for each group:
res$summary$ALL$group_bootstrap_means
#    • A tibble with columns: group, mean_est, ci_lower, ci_upper, and n (sample size).

# 3b. View pairwise effect‐size estimates (mean_diff, cohens_d + their CIs) for every pair of groups:
res$summary$ALL$pairwise_effectsizes
#    • A tibble with: group1, group2, mean_diff, mean_diff_lower, mean_diff_upper,
#                     cohens_d, cohens_d_lower, cohens_d_upper.

# 4. Plotting:
#    - `res$plots$ALL$boot_means_plot` shows each group’s bootstrapped mean ± 95% CI.
#    - `res$plots$ALL$pairwise_effects_plot` shows pairwise estimates (mean_diff & Cohen’s d) ± 95% CI.
res$plots$ALL$boot_means_plot
res$plots$ALL$pairwise_effects_plot

# 5. (Optional) Arrange the two plots side‐by‐side using patchwork:
#    • install.packages("patchwork") if you don’t already have it.
#    • Then:
library(patchwork)
res$plots$ALL$boot_means_plot + res$plots$ALL$pairwise_effects_plot
```
