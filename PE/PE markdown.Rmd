...
#################################Preperation method
pe_rep_final <- read.csv("~/GitHub/TEdissy_analysis/PE/output PE/export data/Samples Analysis Final/PErep_final.csv")
PE_methods <- pe_rep_final %>%
  dplyr::filter(Location %in% c(
    "Lima Market Freeze Dry", 
    "Ilo Freeze Dry", 
    "Ilo oven dry", 
    "Ilo mariculture", 
    "Ilo Fresh", 
    "Lima Market Fresh", 
    "Lima Market Oven Dry"
  )) %>%
  dplyr::mutate(
    Location = dplyr::recode(Location,
      "Ilo mariculture" = "Ilo oven dry"
    )
  )
#
# Perform Kruskal-Wallis test
kw_result <- kruskal.test(
  formula = PE_mg_per_g_sample_mean ~ Location,
  data = PE_methods
)
...
# Optionally, extract elements for reporting:
kw_summary <- tibble::tibble(
